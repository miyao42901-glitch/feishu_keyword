...
class DouyinSearchResult(Base):
    """与 `schema.sql` 中 `feishu_douyin_results` 一致（无 `xsec_token` 列）。"""

    __tablename__ = "feishu_douyin_results"
    __table_args__ = (
        UniqueConstraint("task_id", "post_id", name="uq_feishu_douyin_results_task_post"),
    ...
    verify_name: Mapped[str] = mapped_column(String(128), nullable=False, default="")
