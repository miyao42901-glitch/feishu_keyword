-- 目的: 开发/联调用种子数据 (超级管理员 + 角色 + 菜单绑定)
-- 警告: 默认密码见下方注释; 生产执行前必须改密或删除本文件中的 INSERT
-- 状态: 仅脚本入仓; 未在共用 RDS 执行前勿登记「已执行」
-- 依赖: 必须先执行 20260428_01_create_sys_backoffice_rbac.sql
-- 默认口令: Admin123a (bcrypt 见 password_hash; 符合 utils/password_policy 强度)
