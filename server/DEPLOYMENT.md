...
EnvironmentFile=/opt/feishu_keyword/server/.env
Environment=HTTP_WORKERS=4
ExecStart=/opt/feishu_keyword/server/.venv/bin/gunicorn http_service:app \
...
WorkingDirectory=/opt/feishu_keyword/python
EnvironmentFile=/opt/feishu_keyword/server/.env
ExecStart=/opt/feishu_keyword/server/.venv/bin/celery \
...
可多阶段镜像：基础镜像 Python 3.9，复制 `server/`，`pip install -r requirements-http.txt`，分别用 `CMD` 跑 HTTP 与 Celery（两个 service 共用 image、不同 command）。挂载 `.env` 或 K8s Secret。Broker/DB 用 compose 网络内 Redis、MySQL 服务名。
