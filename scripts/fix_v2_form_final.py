...
# -*- coding: utf-8 -*-
"""
Fix V2 form to match Douyin structure exactly
"""
...
def refactor_v2_form():
    """Refactor V2 form to match Douyin structure exactly"""
  ...
  <div class="collect-panel">
    ...
    <div class="collect-sub-panel">
      <div class="section-block">
        <el-input v-model="paneData.keywords" placeholder="输入视频号名称，或选择已有表格" />
      </div>
    </div>
    ...
    <div class="collect-btn-container">
      <div class="collect-btn-item">
        <el-button class="collect-btn" :disabled="isLocked || !formData.key || (paneData.getDataType === 0 && !paneData.keywords)" @click="paneData.getDataType === 0 ? upsertUser() : getRecentWorks(paneData.searchRange, paneData.getWorksType)">
          {{ paneData.getDataType === 0 ? '采集数据' : '采集数据' }}
        </el-button>
      </div>
...
'''
    ...
    print("V2 form refactored successfully!")
    print("Now matches Douyin structure exactly with question icon!")
    return True
